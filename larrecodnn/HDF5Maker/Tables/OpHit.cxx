...
{
  "run", "subrun", "event", "oh_id", "flash_id", "channel", "time", "area",
  "width", "amplitude"
};
...
void OpHitTable::Fill(art::Event const& evt)
{
  ...
  for (size_t oh_id = 0; oh_id < hits->size(); ++oh_id) {
    ...
    fData.push_back({
      id.run(), id.subRun(), id.event(), oh_id,  // event ID, oh_id
      flash_id, hit.OpChannel(), hit.PeakTime(), // flash_id, channel, time
