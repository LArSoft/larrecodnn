...
std::vector<std::string> static const FlashColumns
{
  "run", "subrun", "event", "flash_id", "pe", "position_y", "position_z",
  "position_t", "width_y", "width_z", "width_t"
};

//-----------------------------------------------------------------------------
// flash table constructor
FlashTable::FlashTable(std::string const& flashLabel,
...
void FlashTable::Fill(art::Event const& evt)
{
  ...
  for (unsigned int flash_id = 0; flash_id < flashes->size(); ++flash_id) {
    ...
    fData.push_back({
      ...
      flash_id, f.TotalPE(),                // flash_id, pe
      f.YCenter(), f.ZCenter(), f.Time(),   // position
      f.YWidth(), f.ZWidth(), f.TimeWidth() // width
    });
