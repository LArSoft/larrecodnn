...
std::vector<std::string> static const LDepColumns
{
  "run", "subrun", "event", "oh_id", "g4_id", "energy",
  "position_x", "position_y", "position_z"
};

//-----------------------------------------------------------------------------
// light deposit table constructor
LDepTable::LDepTable(std::string const& opHitLabel,
  ...
  for (size_t oh_id = 0; oh_id < hits->size(); ++oh_id) {
    ...
    for (const sim::TrackSDP& sdp : bt->OpHitToEveTrackSDPs(hit)) {
      ...
      if (sdp.trackID < 0) {
        ...
        << "Negative track ID (" << sdp.trackID << ") found in simulated "
      ...
      fData.push_back({
        ...
        oh_id, sdp.trackID, sdp.energy,     // oh_id, g4_id, energy
        xyz[0], xyz[1], xyz[2]              // position
      });
