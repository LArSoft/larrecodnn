...
HDF5Maker::HDF5Maker(fhicl::ParameterSet const& p)
  : EDAnalyzer{p},
    fNuLabel(p.get<std::string>("NuLabel")),
    ...
    fOutputName(p.get<std::string>("OutputName"))
{
  ...
  fTables.push_back(std::make_unique<NeutrinoTable>(fNuLabel));
...
//-----------------------------------------------------------------------------
void HDF5Maker::analyze(const art::Event& evt)
{
  // fill tables from art record and write to HDF5 file
  for (const std::unique_ptr<ITable>& table : fTables) {
    table->Fill(evt);
    table->WriteNtuple();
  } // for table
